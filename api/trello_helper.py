...
class TrelloHelper(ITaskDataManager):
    ...
    def get_cards(self) -> List[Card]:
        response_json = self.api.get(self._build_board_url("cards"))
        return [Card.json_to_card(item) for item in response_json]

    def get_card(self, id) -> Card:
        response_json = self.api.get(self._build_board_url(f"cards/{id}"))
    ...
    def add_card(self, title, list_id, due) -> None:
        ...
        return self.api.post(self._build_card_url(""), params=params)

    def delete_card(self, id) -> None:
        return self.api.delete(self._build_card_url(id))

    def get_lists(self) -> List[CardList]:
        response_json = self.api.get(self._build_board_url("lists"))
    ...
    def move_card_to_list(self, card_id, list_id) -> None:
        ...
        return self.api.put(self._build_card_url(card_id), params=params)

    def _build_board_url(self,uri) -> str:
        return f"{self.config.BASE_URL}/boards/{self.config.BOARD_ID}/{uri}"

    def _build_card_url(self,uri) -> str:
